...
class OVGNetNode:
    def __init__(self):
        ...
        self.grasping_output_dir = rospy.get_param('~grasping_output_dir', None)
        ...
        os.makedirs(self.grasping_output_dir, exist_ok=True)
        self.identifier = 0
    ...
    def linguisticc_callback(self, msg):
        self.groundingdino_text_prompt = msg.data
    ...
    def wait_and_run_graspnet(self):
        ...
        rospy.loginfo(f"All processing complete. Running GraspNet ...")
        ...
        try:
            fuse_pcd, best_pose, best_score = get_graspnet_inference(
                ...
                output_dir=self.grasping_output_dir,
                frame_id=frame_id,
                visualize=True
            )

            rospy.loginfo(f"Completed GraspNet inference on frame ...")
            ...
            if pose_msg:
                rospy.loginfo(f'Publish the best grasping pose generated by Graspnet on Topic: /best_grasp_pose')
                self.best_grasp_pose_pub.publish(pose_msg)
            else:
                rospy.logwarn("Best Grasp Pose not Found")
        except Exception as e:
            rospy.logerr(f"Failed GraspNet inference on frame: {e}")
            if self.use_gpu:
                torch.cuda.empty_cache()
    ...
    #     rospy.loginfo(f"All processing complete. Running GraspNet on frame ...")
        
    #     # Run GraspNet on the first successful frame
    #     try:
    #         fuse_pcd, best_pose, best_score = get_graspnet_inference_on_multiview(
    #             checkpoint_path=self.graspnet_checkpoint_path,
    #             refine_approach_dist=self.grapnet_refine_approach_dist,
    #             dist_thresh=self.graspnet_dist_thresh,
    #             angle_thresh=self.graspnet_angle_thresh,
